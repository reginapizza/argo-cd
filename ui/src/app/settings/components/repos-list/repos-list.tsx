...
import {Form, FormValues, FormApi, Text, TextArea} from 'react-form';
...
> {
    ...
    ConnectRepoFormButton(method: string, onSelection: (method: string) => void) {
        return (
    ...
    onChooseDefaultValues(method: string) {
        switch (method) {
            case ConnectionMethod.SSH || ConnectionMethod.HTTPS:
                return {type: 'git'};
    ...
    onValidateErrors(method: string) {
        switch (method) {
            ...
            case ConnectionMethod.GITHUBAPP:
                return (params: NewGitHubAppRepoParams) => ({
                    url: (!params.url && 'Repository URL is required') || (this.credsTemplate && !this.isHTTPSUrl(params.url) && 'Not a valid HTTPS URL'),
            ...
            <Page
                ...
                <SlidingPanel
                    ...
                    <DataLoader load={() => services.projects.list('items.metadata.name').then(projects => projects.map(proj => proj.metadata.name).sort())}>
                        {projects => (
                            <Form
                                onSubmit={this.onSubmitForm(this.state.method)}
                                getApi={api => (this.formApi = api)}
                                defaultValues={this.onChooseDefaultValues(this.state.method)}
                                validateError={this.onValidateErrors(this.state.method)}>
                                {formApi => (
